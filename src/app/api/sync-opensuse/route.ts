...
export async function POST(request: NextRequest) {
  ...
  ;(async () => {
    try {
      ...
      console.log(`✅ Fetched ${packages.length} openSUSE packages`)
      ...
      if (hardDays > 0) {
        ...
        await query(
          ...
          ['opensuse', deleteCutoff]
        )
      }
      
      syncStatus.status = 'complete'
      console.log('✅ openSUSE sync completed successfully')
      
    } catch (error) {
      console.error('❌ openSUSE sync error:', error)
  ...
  return NextResponse.json({ 
    message: 'openSUSE package sync started',
    status: syncStatus 
  })
}
